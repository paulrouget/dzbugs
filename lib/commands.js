...
exports.init = function() {
  ...
  DZ.commandLine.registerCommand({
    ...
    keybinding: {name: "?"}, //FIXME: doesn't work
    validation: "",
  ...
  DZ.commandLine.registerCommand({
    ...
    needsCurrentBugId: true,
    keybinding: {name: "!"}, //FIXME: doesn't work
    validation: "",
    areAgumentsValid: function(argv) { return argv.length == 0; },
    completeArgument: function(index, value, bugId) { return null; },
    exec: function(id) {
      var data = DZ.store.$(id).src.local.data;
      if (!data.tags) data.tags = [];
      var idx = data.tags.indexOf("important");
      if (idx == -1) {
        data.tags.push("important");
      } else {
