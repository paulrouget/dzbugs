...
var layout = {
  ...
  ////////////// MOVES
    ...
    if (this.ensureCursorIsVisible()) this.drawCursorLine(lastCursor);
  },
  home: function() {
    var lastCursor = this.cursor;
    this.cursor = 0;
  ...
  pageDown: function() {
    var lastCursor = this.cursor;
    this.cursor += this.height;
    this.cursor = Math.min(DZ.store.getCount() - 1, this.cursor);
    if (this.ensureCursorIsVisible()) this.drawCursorLine(lastCursor);
  },
  pageUp: function() {
    var lastCursor = this.cursor;
  ...
  drawLine: function(row, xoffset, start, end) {
    ...
    if (totalLength < room) {
      T.pos(room - totalLength + xoffset, row);
