 ...
 *    bugStore.search = function(string) {};
 *    bugStore.getIndexOfBug = function(bugid) {};
...
var bugStore = {
  ...
  search: function(string) {
    var result = filteredAndSortedIndex.filter(function(bugid) {
      var dzbug = this.$(bugid);
      if (dzbug.bz.summary.indexOf(string) > -1)
        return true;
      if (dzbug.bz.id.toString().indexOf(string) > -1)
        return true;
      return false;
    }.bind(this));
    return result;
  ...
  getIndexOfBug: function(id) {
    return filteredAndSortedIndex.indexOf(id);
  },
